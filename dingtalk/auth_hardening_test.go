...
func TestGetAccessTokenEncodesCredentials(t *testing.T) {
	const appKey = "app&admin=true"
	const appSecret = "secret+value%=x"
	client := &http.Client{Transport: dingtalkRoundTripFunc(func(req *http.Request) (*http.Response, error) {
		query := req.URL.Query()
		if query.Get("appkey") != appKey || query.Get("appsecret") != appSecret {
		...
		return dingtalkTokenResponse(req, http.StatusOK, `{"errcode":0,"access_token":"token","expires_in":7200}`), nil
	})}

	if _, err := GetAccessToken(context.Background(), appKey, appSecret, client); err != nil {
...
func TestGetAccessTokenTransportErrorDoesNotLeakCredentials(t *testing.T) {
	...
	client := &http.Client{Transport: dingtalkRoundTripFunc(func(*http.Request) (*http.Response, error) {
	...
	if strings.Contains(err.Error(), secret) || strings.Contains(err.Error(), "appsecret") {
...
func TestGetAccessTokenRejectsInvalidHTTPAndEmptyToken(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			client := &http.Client{Transport: dingtalkRoundTripFunc(func(req *http.Request) (*http.Response, error) {
				return dingtalkTokenResponse(req, tt.status, tt.body), nil
			})}
			_, err := GetAccessToken(context.Background(), "app", "secret", client)
...
func dingtalkTokenResponse(req *http.Request, status int, body string) *http.Response {
