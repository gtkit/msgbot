...
// TokenCache access token 缓存后端，由调用方实现存取逻辑（内存、Redis 均可）。
// 实现必须并发安全。
//
// 重要：一个 TokenCache 实例只能服务一组凭证（appID/appSecret）。
// 多组凭证必须各自创建独立实例，否则 token 会互相覆盖。
// Redis 实现应在构造时绑定唯一 key，例如：
//
//	type redisTokenCache struct {
//	    rdb *redis.Client
//	    key string // 构造时绑定，如 "feishu:token:" + appID
//	}
//
//	func (c *redisTokenCache) Get(ctx context.Context) (*feishu.AccessToken, error) {
//	    data, err := c.rdb.Get(ctx, c.key).Bytes()
//	    if errors.Is(err, redis.Nil) {
//	        return nil, nil // 未命中或已过期.
//	    }
//	    if err != nil {
//	        return nil, err // 建议在此记录错误，库会静默降级为直连.
//	    }
//	    var t feishu.AccessToken
...
//	}
//
//	func (c *redisTokenCache) Set(ctx context.Context, t *feishu.AccessToken, ttl time.Duration) error {
//	    data, err := json.Marshal(t)
//	    if err != nil {
//	        return err
//	    }
//	    return c.rdb.Set(ctx, c.key, data, ttl).Err() // 过期淘汰交给 Redis TTL.
//	}
...
// GetAccessTokenCached 带缓存地获取 token：缓存命中直接返回；
// 未命中请求飞书，按有效期算好 TTL（提前 300 秒）回写缓存后返回。
//
// 行为契约：
//   - 缓存后端故障自动降级为直连获取（Get 错误按未命中处理、Set 错误忽略），
//     不阻断调用；错误被静默吞掉，需要观测时应在自定义 TokenCache 实现内记录。
//   - 同凭证并发未命中时只向飞书发一次请求（防冷启动击穿）。
//   - cache 为 nil 时不持久缓存（等价于 GetAccessToken），但仍保留并发去重。
func GetAccessTokenCached(ctx context.Context, appID, appSecret string,
	...
	return tokencache.Fetch(ctx, cache, tokenGroup,
		tokencache.Key("feishu", appID, appSecret),
		func(ctx context.Context) (*AccessToken, int, error) {
			t, err := GetAccessToken(ctx, appID, appSecret, client...)
			if err != nil {
				return nil, 0, err
			}
			return t, t.Expire, nil
		})
}
