...
func TestGetAccessTokenCached(t *testing.T) {
	...
	client := &http.Client{Transport: wecomRoundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(`{"errcode":0,"errmsg":"ok","access_token":"wecom-tok","expires_in":7200}`)),
			Request:    req,
		}, nil
	})}
	cache := NewMemoryTokenCache()
	...
	for i := range 2 {
		tok, err := GetAccessTokenCached(context.Background(), "cached-corp", "secret", cache, client)
		if err != nil {
			t.Fatalf("call %d: %v", i, err)
		}
		if tok.AccessToken != "wecom-tok" {
