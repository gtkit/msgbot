...
// 实现必须并发安全。
//
// 重要：一个 TokenCache 实例只能服务一组凭证（corpID/corpSecret）。
// 多组凭证必须各自创建独立实例，否则 token 会互相覆盖。
// Redis 实现应在构造时绑定唯一 key（如 "wecom:token:" + corpID），
...
// 未命中请求企业微信，按有效期算好 TTL（提前 300 秒）回写缓存后返回。
// 企业微信对 gettoken 接口有频率限制，生产环境强烈建议使用本函数。
//
// 行为契约：
//   - 缓存后端故障自动降级为直连获取（Get 错误按未命中处理、Set 错误忽略），
//     不阻断调用；错误被静默吞掉，需要观测时应在自定义 TokenCache 实现内记录。
//   - 同凭证并发未命中时只向企业微信发一次请求（防冷启动击穿）。
//   - cache 为 nil 时不持久缓存（等价于 GetAccessToken），但仍保留并发去重。
func GetAccessTokenCached(ctx context.Context, corpID, corpSecret string,
	...
	return tokencache.Fetch(ctx, cache, tokenGroup,
		tokencache.Key("wecom", corpID, corpSecret),
		func(ctx context.Context) (*AccessToken, int, error) {
			t, err := GetAccessToken(ctx, corpID, corpSecret, client...)
			if err != nil {
				return nil, 0, err
			}
